    ...
    <div className="space-y-6">
      ...
      <Steps>
        ...
        >
          ...
          <Callout tone="info" title="Save the app credentials in Unibox">
            Copy <strong>App ID</strong> and <strong>App secret</strong> from <UiPath parts={["App settings", "Basic"]} />,
            then paste them into{" "}
            <Link href="/admin/credentials" className="font-medium text-primary underline underline-offset-2">
              Credentials
            </Link>{" "}
            along with a <strong>verify token</strong> — any string you invent. No environment variables and no
            redeploy: the app belongs to your workspace. The app secret is what proves a webhook really came from
            Meta, so without it Unibox rejects every delivery.
          </Callout>
        </Step>
        ...
        >
          ...
          <p>
            <strong>Verify token:</strong> the one you saved on the Credentials page. Press{" "}
      ...
      <ValueTable
        ...
        rows={[
          ...
          { value: "App id and app secret", where: <UiPath parts={["App settings", "Basic"]} />, goes: <>The <Link href="/admin/credentials" className="text-primary underline underline-offset-2">Credentials</Link> page</> },
          { value: "Verify token", where: "Any string you invent", goes: "Credentials page, and Meta's webhook dialog" }
        ]}
      />

      <Troubleshooting
        rows={[
          { symptom: "The webhook dialog says verification failed", fix: <>Unibox is not reachable at that URL, there is a typo, or the verify token differs from the one saved on the Credentials page.</> },
    ...
    <div className="space-y-6">
      ...
      <Steps>
        ...
        >
          ...
          <p>
            Verify token: the one saved on the Credentials page. After it verifies, subscribe to{" "}
            <code>messages</code> and <code>messaging_postbacks</code>.
          </p>
          <Callout tone="info" title="Path B needs one more secret">
            The Instagram-Login product signs webhooks with its <strong>Instagram app secret</strong>, shown on the
            Instagram product page rather than the app's Basic Settings. Paste it into the optional{" "}
            <strong>Instagram app secret</strong> field on{" "}
            <Link href="/admin/credentials" className="font-medium text-primary underline underline-offset-2">
              Credentials
            </Link>.
          </Callout>
      ...
      <ValueTable
        ...
        rows={[
          ...
          { value: "Instagram app secret", where: "Instagram product page in the Meta dashboard (Path B only)", goes: "Credentials page, optional field (Path B only)" }
        ]}
      />
      ...
      <Troubleshooting
        rows={[
          ...
          { symptom: <>Webhook returns <code>401</code> in the logs (Path B)</>, fix: "The Instagram app secret on the Credentials page is missing or wrong. It is a different value from the app's Basic Settings secret." },
    ...
    <div className="space-y-6">
      ...
      <Steps>
          ...
          art={
            <MockConsole
              ...
              fields={[
                ...
                { label: "Verify token", value: "your verify token", highlight: true },
        ...
        >
          ...
          <p>
            Verify token: the one saved on the Credentials page. Verify, then subscribe to the{" "}
  ...
  -d "verify_token=<YOUR_VERIFY_TOKEN>"`}</pre>
      ...
      <ValueTable
        ...
        rows={[
          ...
          { value: "Verify token", where: "The Credentials page", goes: "Meta's webhook dialog" }
        ]}
      />
    ...
    <div className="space-y-6">
      ...
      <Steps>
        ...
        >
          ...
          <Callout tone="tip" title="Verify before connecting?">
            Verify passes only if Unibox already knows the secret, so connect the channel first (step 5) and then come
      ...
      <ValueTable
        ...
        rows={[
          ...
          { value: "Channel secret", where: "Basic settings tab", goes: "Stored encrypted on the channel" }
        ]}
      />
